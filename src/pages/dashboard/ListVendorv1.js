...
import VendorDetail from 'src/components/common/VendorDetails';
...
export default function ListVendor() {
        ...
        if (perm.create) {
          if (item.ticket_id == null) {
            if (item.is_active == true) {
              return (
                <Button variant="contained" color="error" onClick={handleBtnAct(item)}>
                  <Typography>Deactivate</Typography>
                </Button>
              );
            } else {
              return (
                <Button variant="contained" color="success" onClick={handleBtnAct(item)}>
  ...
  return (
    <>
      ...
      /> */}
      <TableLayout data={data} lengthRow={colLength} header={columns} detailsComp={VendorDetail} />
