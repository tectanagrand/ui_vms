...
import TableRows from './TableRowsv1';
import { useState, useEffect } from 'react';
import SearchBar from '@mkyy/mui-search-bar';

export default function TableLayout({ data, header, lengthRow, detailsComp }) {
  //header format
  /*
  [
    {
      data : {key property of data},
      header : {header name},
      renderCell? : (row) => {
          ...
          <TableBody>
            {data.length != 0 &&
              (rowsPerPage > 0 ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : rows).map(
                (item) => (
                  <TableRows key={item.id} header={header} row={item} length={lengthRow} DetComp={detailsComp} />
                )
              )}
